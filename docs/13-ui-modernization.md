...
- 2026-05-04: Workspace VS Code tasks now resolve `cmake.exe` and `ctest.exe` from the environment, retry by clearing only stale default-config state instead of deleting the whole `build/` tree, and build the UI snapshot harness alongside offline tests in the Debug test path.
...
- 2026-05-04: In this environment, CMake Tools can still be attached to `build/release-selfcontained` while the validated harness flow lives under the default Debug tree. Prefer the repo tasks or the workspace's explicit `cmake.exe` path in `.vscode/settings.json` when validating the snapshot harness.
...
The historical phases above describe the former tab/rail interface. The focused Sound shell supersedes those layout decisions, including persisted utility-tab selection, automatic inspector promotion, the preset strip and dual audition surfaces. See [the current implementation notes](SAMPLER_SIMPLIFICATION_2026-09-07.md) and [user guide](USER_GUIDE.md). Existing engine/state behavior remains the compatibility boundary.
