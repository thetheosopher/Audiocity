...
- `<region>`: `sample`, `lokey`, `hikey`, `key`, `pitch_keycenter`, `lovel`, `hivel`, `transpose`, `tune`, `offset`, `end`, `loop_start`, `loop_end`, `loop_mode`, `volume`, `pan`, `seq_position`, `seq_length`, `xfin_lovel`, `xfin_hivel`, `xfout_lovel`, `xfout_hivel`, `off_by`, `trigger`
...
- Velocity crossfades (`xfin_*`, `xfout_*`) are imported into the zone model and preserved through Mapping edits/state restore.
...
- Unsupported values for known opcodes such as unsupported `trigger` values and unsupported `loop_mode` values emit warnings so imports do not fail silently.
