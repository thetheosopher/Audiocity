...
| MP-4 | In progress | Added archive-relative path validation and traversal-shaped ZIP sample-reference rejection for Bitwig `.multisample` and Korg `.korgmultisample`; added NCW converter quoting coverage; added a malformed importer corpus regression covering SF2, Bitwig, Korg, Ableton, KMP, EXS24, and NN-XT failure paths. | `audiocity_offline_tests` CTest entries passed. Remaining: broader importer bounds audits and, if desired, an external libFuzzer/AFL harness. |
...
- **Status:** In progress as of 2026-05-31. Completed bounded hardening slices for ZIP sample references, NCW converter command handling, and malformed-input corpus coverage: archive paths are validated as relative/no-traversal, Bitwig and Korg archive importers skip unsafe entries, offline regressions cover traversal-shaped sample refs, NCW converter quoting is covered with a shell-metacharacter input-path regression, and a corpus-style offline test covers malformed SF2, Bitwig, Korg, Ableton, KMP, EXS24, and NN-XT inputs. Remaining work includes broader binary/XML bounds audits and, if desired, an external libFuzzer/AFL harness.
