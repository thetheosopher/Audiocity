...
- **Status:** In progress as of 2026-05-31. Completed bounded hardening slices for ZIP sample references, NCW converter command handling, malformed-input corpus coverage, and binary importer chunk bounds: archive paths are validated as relative/no-traversal, Bitwig and Korg archive importers skip unsafe entries, offline regressions cover traversal-shaped sample refs, NCW converter quoting is covered with a shell-metacharacter input-path regression, a corpus-style offline test covers malformed SF2, Bitwig, Korg, Ableton, KMP, EXS24, and NN-XT inputs, KMP `RLP1` entry parsing is capped to declared/format limits, and EXS24 oversized chunks emit truncation diagnostics through overflow-safe arithmetic. Remaining work includes broader binary/XML bounds audits and, if desired, an external libFuzzer/AFL harness.
