...
| MP-1 | Started | Added a prepared-import publish API on the processor and routed SF2/DecentSampler/Bitwig/MPC/1010music/TAL/TX16Wx/Korg/Ableton/disting EX/KMP/EXS24/NN-XT imports through a background worker; results publish back on the message thread and the Load button becomes an import cancel action for stale-job suppression. | `audiocity_ui_snapshot_harness`, `audiocity_preset_runtime_smoke`, and `audiocity_offline_tests` build/run locally; non-UI CTest passed 4/4; UI snapshot harness generated MP-1 check snapshots. Full determinate progress and SFZ/REX/NKI worker coverage remain follow-up work. |
...
- **Status:** Started on 2026-05-31. The first slice prepares selected imported-program formats on a worker thread, publishes prepared `Program`/sample buffers on the message thread, and exposes cancel by invalidating the active import generation. Remaining work: cooperative cancellation inside long decodes, richer/determinate progress, and async coverage for SFZ/REX/NKI/sample load paths.
...
**MP-1 (async import):**
> Continue the started refactor in [src/plugin/PluginEditor.cpp](src/plugin/PluginEditor.cpp#L5303): selected imported-program formats already run parse/decode on a background worker and publish prepared `Program`/sample buffers on the message thread. Finish the milestone by adding cooperative cancellation/progress reporting inside long decodes, moving SFZ/REX/NKI/sample load paths off the message thread, and preserving the audio thread contract in [docs/02-real-time-rules.md](docs/02-real-time-rules.md).
