...
| MP-4 | Done | Added archive-relative path validation and traversal-shaped ZIP sample-reference rejection for Bitwig `.multisample` and Korg `.korgmultisample`; added NCW converter quoting coverage; added malformed importer corpus coverage; capped KMP `RLP1` entry parsing; added overflow-safe EXS24 chunk truncation diagnostics; added SF2 LIST/table validation; capped ZIP manifest/audio and Ableton gzip XML payload reads. | `audiocity_offline_tests` and non-UI CTest entries passed. External libFuzzer/AFL remains optional future DX, but the roadmap acceptance criteria are covered by corpus tests, bounds checks, and traversal rejection. |
| MP-1 | Done | Added processor-side prepared sample/import publish APIs and routed sample files plus SFZ/REX/NKI/SF2/DecentSampler/Bitwig/MPC/1010music/TAL/TX16Wx/Korg/Ableton/disting EX/KMP/EXS24/NN-XT loads through a background worker; results publish back on the message thread, the Load button becomes a cancel action, and the NKI locate-samples retry path is async too. | `audiocity_ui_snapshot_harness`, `audiocity_preset_runtime_smoke`, and `audiocity_offline_tests` build/run locally; non-UI CTest passed 4/4; UI snapshot harness generated MP-1 finish-check snapshots. |
| MP-2 | In progress | Landed three editor-split slices by extracting the About, Generate, and Capture tabs into standalone page components, with `PluginEditor` now hosting those surfaces while preserving behavior. | `audiocity_ui_snapshot_harness` generated the `generate` and `capture` snapshots and `audiocity_offline_tests` passed locally; Debug test targets build. |
| MP-3 | In progress | Added `src/engine/DecentSamplerExporter.{h,cpp}`, processor/editor `.dspreset` save routing, offline save-routing plus round-trip coverage, preserved DecentSampler group volume/pan/release-trigger defaults across import/export instead of flattening them into zones, and now round-trips documented round-robin metadata plus choke-group semantics. | `audiocity_offline_tests` passed locally; `CMake: Build Tests (Debug)` succeeded for the updated exporter/importer/tests slice. |
...
- **Status:** Done on 2026-05-31. All `loadFileAsInstrument` paths now prepare on a worker thread and publish on the message thread, including sample files, SFZ, REX, and legacy NKI retry imports. Cancel invalidates stale jobs cleanly from the UI and the status label surfaces preparing/publishing progress while the audio thread contract stays unchanged.
- **Problem:** Importing large multisamples/libraries runs synchronously on the message thread, freezing the UI with no feedback.
- **Approach:** Move parse/decode onto a background worker job; show a determinate/indeterminate progress affordance and a cancel button; marshal the finished `Program` + sample buffers back to the message thread and swap atomically into the engine. Reuse the existing snapshot-swap path so the audio thread is untouched.
...
- **Status:** In progress on 2026-05-31. Three extraction slices are landed: the About, Generate, and Capture tabs now live in standalone page components while the remaining tabs still live in `PluginEditor.cpp`.
...
- **Status:** In progress on 2026-05-31. The exporter/save path now preserves grouped volume/pan/release-trigger defaults, documented round-robin metadata (`seqMode` / `seqPosition` / `seqLength`), and choke-group semantics through DecentSampler round-trips; unsupported semantics still include velocity fades, one-shot triggers, and sustain-loop differentiation.
...
- **Status:** Done on 2026-05-31. Completed bounded hardening slices for ZIP sample references, NCW converter command handling, malformed-input corpus coverage, binary importer chunk bounds, SF2 container/table validation, and bounded archive/GZIP reads: archive paths are validated as relative/no-traversal, Bitwig and Korg archive importers skip unsafe entries, offline regressions cover traversal-shaped sample refs, NCW converter quoting is covered with a shell-metacharacter input-path regression, a corpus-style offline test covers malformed SF2, Bitwig, Korg, Ableton, KMP, EXS24, and NN-XT inputs, KMP `RLP1` entry parsing is capped to declared/format limits, EXS24 oversized chunks emit truncation diagnostics through overflow-safe arithmetic, SF2 short LIST chunks and empty required tables are rejected, and Bitwig/Korg ZIP plus Ableton gzip whole-entry reads are capped before allocation.
...
**MP-1 (async import):**
> Completed on 2026-05-31. [src/plugin/PluginEditor.cpp](src/plugin/PluginEditor.cpp#L5303) now routes sample files and all supported imported-program formats through a background worker that prepares data off-thread and publishes on the message thread via processor-owned prepared-load APIs, while preserving the audio thread contract in [docs/02-real-time-rules.md](docs/02-real-time-rules.md).

**MP-2 (editor split):**
> Started on 2026-05-31. The About, Generate, and Capture tabs are now extracted into standalone page components and hosted by [src/plugin/PluginEditor.cpp](src/plugin/PluginEditor.cpp) as the first three decomposition slices. Continue with the remaining tabs (Sample, Mapping, Player, Library) while preserving behavior and keeping UI snapshot baselines under [tests/ui-snapshot-baselines](tests/ui-snapshot-baselines) unchanged.

**MP-3 (DS exporter):**
> Started on 2026-05-31. [src/engine/DecentSamplerExporter.cpp](src/engine/DecentSamplerExporter.cpp) now exists, `PluginProcessor`/`PluginEditor` route `.dspreset` saves through it, and offline tests cover save routing plus import→export→re-import round trips. The current slice preserves DecentSampler group volume/pan/release-trigger defaults via `<groups>/<group>`, documented round-robin metadata via `seqMode` / `seqPosition` / `seqLength`, and choke-group semantics via tag-based silencing plus Audiocity round-trip metadata; follow-up work remains for velocity fades, one-shot triggers, and sustain-loop differentiation.
